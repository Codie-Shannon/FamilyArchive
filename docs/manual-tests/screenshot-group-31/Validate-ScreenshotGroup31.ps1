...
$evidenceDir = Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-31'
$desktop = @(
    '01_v360_Family_Chat_Home.png',
    '02_v360_New_Conversation.png',
    '03_v360_Family_Conversation.png',
    '04_v360_Conversation_Controls.png',
    '06_v360_Reported_Message_Exception.png'
)
$responsive = '05_v360_Responsive_Family_Chat.png'
$privacyTextTargets = @(
    (Join-Path $repoRoot 'app\Domain\Communication'),
    (Join-Path $repoRoot 'app\Http\Controllers\FamilyMessageController.php'),
    (Join-Path $repoRoot 'database\seeders\ScreenshotGroup31DemoSeeder.php'),
    (Join-Path $repoRoot 'resources\js\family-chat.js'),
    (Join-Path $repoRoot 'resources\views\components\family-chat.blade.php'),
    (Join-Path $repoRoot 'tests\Feature\Group31'),
    (Join-Path $repoRoot 'docs\manual-tests\screenshot-group-31'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-31')
)
$forbiddenTokens = @(
    ('co' + 'dex'),
    ('chat' + 'gpt'),
...
try {
    Write-Host '=== Screenshot Group 31 focused tests ===' -ForegroundColor Cyan
    & php vendor/pestphp/pest/bin/pest tests/Feature/Group31/EmbeddedFamilyMessagingTest.php tests/Feature/Group24/DelegatedFamilyOperationsTest.php --compact
    ...
    & php vendor/bin/pint --test app/Domain/Communication app/Http/Controllers/FamilyMessageController.php database/seeders/ScreenshotGroup31DemoSeeder.php tests/Feature/Group31/EmbeddedFamilyMessagingTest.php
    ...
    & php -d memory_limit=1G vendor/bin/phpstan analyse app/Domain/Communication app/Http/Controllers/FamilyMessageController.php database/seeders/ScreenshotGroup31DemoSeeder.php --no-progress
    ...
    Write-Host 'OK  SG31 text contains no forbidden private or tooling references.' -ForegroundColor Green

    Write-Host "`n=== Repository evidence state ===" -ForegroundColor Cyan
    & git status --short

    Write-Host "`nScreenshot Group 31 validation commands passed." -ForegroundColor Green
    Write-Host 'Capture this PowerShell window as 07_v360_Validation.png.' -ForegroundColor Yellow
}
finally {
    Pop-Location
}
