...
$evidenceDir = Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-36'
$desktop = @(
    '01_v410_Clipping_Safe_Order.png',
    '02_v410_Independent_Photo_Rotation.png',
    '03_v410_Rotated_Preview_Geometry.png',
    '04_v410_Manual_Rotation_Override.png',
    '05_v410_Immutable_Source_Lineage.png'
)
$responsive = '06_v410_Responsive_Split_Review.png'
$privacyTextTargets = @(
    ...
    (Join-Path $repoRoot 'app\Domain\Processing\Services\PhotoSplitCandidateRenderer.php'),
    (Join-Path $repoRoot 'app\Domain\Processing\Services\PhotoSplitReviewService.php'),
    (Join-Path $repoRoot 'app\Domain\Processing\ValueObjects\RenderedSplitPhoto.php'),
    (Join-Path $repoRoot 'database\seeders\ScreenshotGroup36DemoSeeder.php'),
    (Join-Path $repoRoot 'resources\views\intake\photo-split-editor.blade.php'),
    (Join-Path $repoRoot 'tests\Feature\Group34'),
    (Join-Path $repoRoot 'docs\manual-tests\screenshot-group-36'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-36')
)
$forbiddenTokens = @(
    ('co' + 'dex'),
    ('chat' + 'gpt')
...
try {
    Write-Host '=== Screenshot Group 36 focused tests ===' -ForegroundColor Cyan
    & php vendor/pestphp/pest/bin/pest tests/Feature/Group34 tests/Unit/Domain/Processing/MultiPhotoLayoutDetectorTest.php --compact
    ...
    & php vendor/bin/pint --test app/Domain/Processing/Models/PhotoSplitRegion.php app/Domain/Processing/Services/MultiPhotoLayoutDetector.php app/Domain/Processing/Services/PhotoSplitCandidateRenderer.php app/Domain/Processing/Services/PhotoSplitReviewService.php app/Domain/Processing/ValueObjects/RenderedSplitPhoto.php database/migrations/2026_08_04_090000_add_rotation_to_photo_split_regions.php database/seeders/ScreenshotGroup36DemoSeeder.php tests/Feature/Group34 tests/Unit/Domain/Processing/MultiPhotoLayoutDetectorTest.php
    ...
    Write-Host 'OK  SG36 text contains no forbidden private or tooling references.' -ForegroundColor Green

    Write-Host "`n=== Repository evidence state ===" -ForegroundColor Cyan
    & git status --short

    Write-Host "`nScreenshot Group 36 validation commands passed." -ForegroundColor Green
    Write-Host 'Capture this PowerShell window as 07_v410_Validation.png.' -ForegroundColor Yellow
}
finally {
    Pop-Location
}
