...
$evidenceDir = Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-28'
$desktop = @(
    '01_v330_Album_Library.png',
    '02_v330_Curated_Album.png',
    '03_v330_Event_Album.png',
    '04_v330_Create_Album.png',
    '05_v330_Batch_Add_To_Album.png',
    '06_v330_Archive_Search.png'
)
$responsive = '07_v330_Responsive_Albums.png'
$privacyTextTargets = @(
    (Join-Path $repoRoot 'app\Domain\Knowledge\Enums\ArchiveAlbumType.php'),
    (Join-Path $repoRoot 'app\Domain\Knowledge\Models\CuratedCollection.php'),
    (Join-Path $repoRoot 'app\Domain\Knowledge\ReadModels\ArchiveAlbumSummary.php'),
    (Join-Path $repoRoot 'app\Domain\Knowledge\Services\ArchiveAlbumExplorer.php'),
    (Join-Path $repoRoot 'app\Domain\Browsing\Queries\ApprovedPhotoGalleryQuery.php'),
    (Join-Path $repoRoot 'app\Http\Controllers\Archive\ArchiveAlbumController.php'),
    (Join-Path $repoRoot 'database\seeders\ScreenshotGroup28DemoSeeder.php'),
    (Join-Path $repoRoot 'resources\views\archive\albums'),
    (Join-Path $repoRoot 'resources\views\archive\knowledge.blade.php'),
    (Join-Path $repoRoot 'tests\Feature\Group28'),
    (Join-Path $repoRoot 'docs\manual-tests\screenshot-group-28'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-28\README.md'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-28\Evidence_Index.md')
)
$forbiddenTokens = @(
    ('co' + 'dex'),
    ('chat' + 'gpt'),
...
try {
    Write-Host '=== Screenshot Group 28 focused tests ===' -ForegroundColor Cyan
    & php vendor/pestphp/pest/bin/pest tests/Feature/Group28/AlbumCentredArchiveTest.php tests/Feature/Group23/UnifiedArchiveExperienceTest.php tests/Feature/Group27/InteractiveArchiveMapTest.php --compact
    ...
    & php vendor/bin/pint --test app/Domain/Knowledge/Services/ArchiveAlbumExplorer.php app/Domain/Browsing/Queries/ApprovedPhotoGalleryQuery.php app/Http/Controllers/Archive/ArchiveAlbumController.php database/seeders/ScreenshotGroup28DemoSeeder.php tests/Feature/Group28/AlbumCentredArchiveTest.php
    ...
    & php -d memory_limit=1G vendor/bin/phpstan analyse app/Domain/Knowledge/Services/ArchiveAlbumExplorer.php app/Domain/Browsing/Queries/ApprovedPhotoGalleryQuery.php app/Http/Controllers/Archive/ArchiveAlbumController.php database/seeders/ScreenshotGroup28DemoSeeder.php --no-progress
    ...
    Write-Host 'OK  SG28 text contains no forbidden private or tooling references.' -ForegroundColor Green

    Write-Host "`n=== Repository evidence state ===" -ForegroundColor Cyan
    & git status --short

    Write-Host "`nScreenshot Group 28 validation commands passed." -ForegroundColor Green
    Write-Host 'Capture this PowerShell window as 08_v330_Validation.png.' -ForegroundColor Yellow
}
finally {
    Pop-Location
}
