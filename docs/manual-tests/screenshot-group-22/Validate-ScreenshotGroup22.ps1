...
$evidenceDir = Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-22'
$desktop = @(
    '01_v270_Simplified_Batch_Start.png',
    '02_v270_Browser_Batch_Ready.png',
    '03_v270_Delegated_Intake_Home.png',
    '04_v270_Original_And_Suggested.png',
    '05_v270_Owner_Exception_Queue.png'
)
$responsive = '06_v270_Responsive_Delegated_Review.png'
$privacyTextTargets = @(
    (Join-Path $repoRoot 'app\Domain\CloudImport\Services\BrowserUploadBatch.php'),
    (Join-Path $repoRoot 'app\Domain\CloudImport\Services\TrustedBatchReview.php'),
    (Join-Path $repoRoot 'app\Http\Controllers\ContributorSubmissionController.php'),
    (Join-Path $repoRoot 'app\Http\Controllers\Intake'),
    (Join-Path $repoRoot 'database\seeders\ScreenshotGroup22DemoSeeder.php'),
    (Join-Path $repoRoot 'resources\views\contributor'),
    (Join-Path $repoRoot 'resources\views\intake'),
    (Join-Path $repoRoot 'tests\Feature\Group22'),
    (Join-Path $repoRoot 'docs\manual-tests\screenshot-group-22'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-22\README.md'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-22\Evidence_Index.md')
)
$forbiddenTokens = @(
    ('co' + 'dex'),
    ('chat' + 'gpt'),
...
try {
    Write-Host '=== Screenshot Group 22 focused tests ===' -ForegroundColor Cyan
    & php vendor/pestphp/pest/bin/pest tests/Feature/Group22/DelegatedIntakeCompletionTest.php tests/Feature/Intake/TrustedBatchReviewTest.php --compact
    if ($LASTEXITCODE -ne 0) {
        throw "Focused tests failed with exit code $LASTEXITCODE."
    }

    Write-Host "`n=== Formatting verification ===" -ForegroundColor Cyan
    & php vendor/bin/pint --test app/Domain/Access/Models/UploadSession.php app/Domain/CloudImport/Services/BrowserUploadBatch.php app/Domain/CloudImport/Services/TrustedBatchReview.php app/Http/Controllers/Admin/OwnerCommandCentreController.php app/Http/Controllers/ContributorSubmissionController.php app/Http/Controllers/Intake app/Http/Controllers/PublicConversationController.php database/migrations/2026_08_01_160000_connect_browser_uploads_to_batch_review.php database/seeders/ScreenshotGroup22DemoSeeder.php tests/Feature/Group22/DelegatedIntakeCompletionTest.php
    if ($LASTEXITCODE -ne 0) {
        throw "Formatting verification failed with exit code $LASTEXITCODE."
    }

    Write-Host "`n=== Static analysis ===" -ForegroundColor Cyan
    & php -d memory_limit=1G vendor/bin/phpstan analyse app/Domain/Access/Models/UploadSession.php app/Domain/CloudImport/Services/BrowserUploadBatch.php app/Domain/CloudImport/Services/TrustedBatchReview.php app/Http/Controllers/Admin/OwnerCommandCentreController.php app/Http/Controllers/ContributorSubmissionController.php app/Http/Controllers/Intake app/Http/Controllers/PublicConversationController.php --no-progress
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host 'OK  SG22 text contains no forbidden private or tooling references.' -ForegroundColor Green

    Write-Host "`n=== Repository evidence state ===" -ForegroundColor Cyan
    & git status --short

    Write-Host "`nScreenshot Group 22 validation commands passed." -ForegroundColor Green
    Write-Host 'Capture this PowerShell window as 07_v270_Validation.png.' -ForegroundColor Yellow
}
finally {
    Pop-Location
}
