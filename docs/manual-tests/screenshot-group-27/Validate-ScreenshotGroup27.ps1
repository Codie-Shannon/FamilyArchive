...
$evidenceDir = Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-27'
$desktop = @(
    '01_v320_Interactive_Archive_Map.png',
    '02_v320_Map_Marker_Detail.png',
    '03_v320_Reviewed_Places_Navigation.png',
    '04_v320_Event_Detail_Navigation.png'
)
$responsive = '05_v320_Responsive_Archive_Map.png'
$privacyTextTargets = @(
    (Join-Path $repoRoot 'app\Http\Controllers\PublicDiscoveryController.php'),
    (Join-Path $repoRoot 'app\Domain\PublicDiscovery\Services\PublicMapPolicy.php'),
    (Join-Path $repoRoot 'database\seeders\ScreenshotGroup27DemoSeeder.php'),
    (Join-Path $repoRoot 'resources\views\components\archive-navigation.blade.php'),
    (Join-Path $repoRoot 'resources\views\components\archive-shell.blade.php'),
    (Join-Path $repoRoot 'resources\views\public-discovery\map.blade.php'),
    (Join-Path $repoRoot 'tests\Feature\Group27'),
    (Join-Path $repoRoot 'docs\manual-tests\screenshot-group-27'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-27\README.md'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-27\Evidence_Index.md')
)
$forbiddenTokens = @(
    ('co' + 'dex'),
    ('chat' + 'gpt'),
...
try {
    Write-Host '=== Screenshot Group 27 focused tests ===' -ForegroundColor Cyan
    & php vendor/pestphp/pest/bin/pest tests/Feature/Group27/InteractiveArchiveMapTest.php tests/Feature/Group18/ArchiveExplorationNavigationTest.php tests/Feature/Group23/UnifiedArchiveExperienceTest.php tests/Feature/Release/PublicDiscoveryReleaseTest.php --compact
    ...
    & php vendor/bin/pint --test app/Http/Controllers/PublicDiscoveryController.php database/seeders/ScreenshotGroup27DemoSeeder.php tests/Feature/Group27/InteractiveArchiveMapTest.php
    ...
    & php -d memory_limit=1G vendor/bin/phpstan analyse app/Http/Controllers/PublicDiscoveryController.php database/seeders/ScreenshotGroup27DemoSeeder.php --no-progress
    ...
    Write-Host 'OK  SG27 text contains no forbidden private or tooling references.' -ForegroundColor Green

    Write-Host "`n=== Repository evidence state ===" -ForegroundColor Cyan
    & git status --short

    Write-Host "`nScreenshot Group 27 validation commands passed." -ForegroundColor Green
    Write-Host 'Capture this PowerShell window as 06_v320_Validation.png.' -ForegroundColor Yellow
}
finally {
    Pop-Location
}
