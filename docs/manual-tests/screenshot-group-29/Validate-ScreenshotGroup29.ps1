...
$evidenceDir = Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-29'
$desktop = @(
    '01_v340_Flexible_Login.png',
    '02_v340_Access_Code.png',
    '03_v340_Guided_Setup_And_Recovery.png',
    '04_v340_Printable_Access_Card.png',
    '05_v340_Approval_Waiting.png'
)
$responsive = '06_v340_Responsive_Access.png'
$privacyTextTargets = @(
    (Join-Path $repoRoot 'app\Domain\Access\Services\MemberAccessService.php'),
    (Join-Path $repoRoot 'app\Http\Controllers\Auth\InvitationController.php'),
    (Join-Path $repoRoot 'app\Http\Controllers\Admin\FamilyOperationsController.php'),
    (Join-Path $repoRoot 'database\seeders\ScreenshotGroup29DemoSeeder.php'),
    (Join-Path $repoRoot 'resources\views\auth'),
    (Join-Path $repoRoot 'resources\views\admin\family-operations.blade.php'),
    (Join-Path $repoRoot 'tests\Feature\Group29'),
    (Join-Path $repoRoot 'docs\manual-tests\screenshot-group-29'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-29')
)
$forbiddenTokens = @(
    ('co' + 'dex'),
    ('chat' + 'gpt'),
...
try {
    Write-Host '=== Screenshot Group 29 focused tests ===' -ForegroundColor Cyan
    & php vendor/pestphp/pest/bin/pest tests/Feature/Group29/GuidedFamilyAccessTest.php tests/Feature/Group12/AccountsContributorAccessTest.php --compact
    ...
    & php vendor/bin/pint --test app/Domain/Access/Services/MemberAccessService.php app/Http/Controllers/Auth/InvitationController.php app/Http/Controllers/Admin/AccountAccessController.php app/Http/Controllers/Admin/FamilyOperationsController.php app/Providers/FortifyServiceProvider.php database/seeders/ScreenshotGroup29DemoSeeder.php tests/Feature/Group29/GuidedFamilyAccessTest.php
    ...
    & php -d memory_limit=1G vendor/bin/phpstan analyse app/Domain/Access/Services/MemberAccessService.php app/Http/Controllers/Auth/InvitationController.php app/Http/Controllers/Admin/AccountAccessController.php app/Http/Controllers/Admin/FamilyOperationsController.php app/Providers/FortifyServiceProvider.php database/seeders/ScreenshotGroup29DemoSeeder.php --no-progress
    ...
    Write-Host 'OK  SG29 text contains no forbidden private or tooling references.' -ForegroundColor Green

    Write-Host "`n=== Repository evidence state ===" -ForegroundColor Cyan
    & git status --short

    Write-Host "`nScreenshot Group 29 validation commands passed." -ForegroundColor Green
    Write-Host 'Capture this PowerShell window as 07_v340_Validation.png.' -ForegroundColor Yellow
}
finally {
    Pop-Location
}
