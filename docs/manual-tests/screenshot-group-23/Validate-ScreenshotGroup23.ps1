...
$evidenceDir = Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-23'
$desktop = @(
    '01_v280_Unified_Archive_Home.png',
    '02_v280_Reviewed_People.png',
    '03_v280_Places_And_Map.png',
    '04_v280_Event_Context.png',
    '05_v280_Family_Branch.png',
    '06_v280_Permission_Aware_Search.png'
)
$responsive = '07_v280_Responsive_Archive_Navigation.png'
$privacyTextTargets = @(
    (Join-Path $repoRoot 'app\Domain\Knowledge\Services'),
    (Join-Path $repoRoot 'app\Http\Controllers\Archive'),
    (Join-Path $repoRoot 'database\seeders\ScreenshotGroup23DemoSeeder.php'),
    (Join-Path $repoRoot 'resources\views\archive'),
    (Join-Path $repoRoot 'resources\views\components\archive-navigation.blade.php'),
    (Join-Path $repoRoot 'tests\Feature\Group23'),
    (Join-Path $repoRoot 'docs\manual-tests\screenshot-group-23'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-23\README.md'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-23\Evidence_Index.md')
)
$forbiddenTokens = @(
    ('co' + 'dex'),
    ('chat' + 'gpt'),
...
try {
    Write-Host '=== Screenshot Group 23 focused tests ===' -ForegroundColor Cyan
    & php vendor/pestphp/pest/bin/pest tests/Feature/Group23/UnifiedArchiveExperienceTest.php tests/Feature/Group18/ArchiveExplorationNavigationTest.php tests/Feature/Group16/UnifiedMemberExperienceTest.php --compact
    if ($LASTEXITCODE -ne 0) {
        throw "Focused tests failed with exit code $LASTEXITCODE."
    }

    Write-Host "`n=== Formatting verification ===" -ForegroundColor Cyan
    & php vendor/bin/pint --test app/Domain/Knowledge/Services/ArchiveKnowledgeAccess.php app/Domain/Knowledge/Services/ArchiveKnowledgeExplorer.php app/Http/Controllers/Archive database/seeders/ScreenshotGroup23DemoSeeder.php tests/Feature/Group23/UnifiedArchiveExperienceTest.php
    if ($LASTEXITCODE -ne 0) {
        throw "Formatting verification failed with exit code $LASTEXITCODE."
    }

    Write-Host "`n=== Static analysis ===" -ForegroundColor Cyan
    & php -d memory_limit=1G vendor/bin/phpstan analyse app/Domain/Knowledge/Services/ArchiveKnowledgeAccess.php app/Domain/Knowledge/Services/ArchiveKnowledgeExplorer.php app/Http/Controllers/Archive --no-progress
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host 'OK  SG23 text contains no forbidden private or tooling references.' -ForegroundColor Green

    Write-Host "`n=== Repository evidence state ===" -ForegroundColor Cyan
    & git status --short

    Write-Host "`nScreenshot Group 23 validation commands passed." -ForegroundColor Green
    Write-Host 'Capture this PowerShell window as 08_v280_Validation.png.' -ForegroundColor Yellow
}
finally {
    Pop-Location
}
