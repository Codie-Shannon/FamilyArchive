...
$evidenceDir = Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-33'
$desktop = @(
    '01_v380_Thirty_Thousand_Qualification.png',
    '02_v380_Interrupted_Run_Recovery.png',
    '03_v380_Checkpoint_Reconciliation.png',
    '04_v380_Private_Migration_Boundary.png'
)
$responsive = '05_v380_Responsive_Qualification.png'
$privacyTextTargets = @(
    (Join-Path $repoRoot 'app\Console\Commands\QualifyArchiveMigrationCommand.php'),
    (Join-Path $repoRoot 'app\Domain\CloudImport\Models\MigrationQualificationRun.php'),
    (Join-Path $repoRoot 'app\Domain\CloudImport\Services\ArchiveMigrationQualification.php'),
    (Join-Path $repoRoot 'app\Http\Controllers\Admin\MigrationQualificationController.php'),
    (Join-Path $repoRoot 'database\seeders\ScreenshotGroup33DemoSeeder.php'),
    (Join-Path $repoRoot 'resources\views\admin\migration-qualification.blade.php'),
    (Join-Path $repoRoot 'tests\Feature\Group33'),
    (Join-Path $repoRoot 'docs\manual-tests\screenshot-group-33'),
    (Join-Path $repoRoot 'docs\screenshot-groups\screenshot-group-33')
)
$forbiddenTokens = @(
    ('co' + 'dex'),
    ('chat' + 'gpt'),
...
try {
    Write-Host '=== Screenshot Group 33 focused tests ===' -ForegroundColor Cyan
    & php vendor/pestphp/pest/bin/pest tests/Feature/Group33/MigrationQualificationTest.php --compact
    ...
    & php vendor/bin/pint --test app/Console/Commands/QualifyArchiveMigrationCommand.php app/Domain/CloudImport/Models/MigrationQualificationRun.php app/Domain/CloudImport/Services/ArchiveMigrationQualification.php app/Http/Controllers/Admin/MigrationQualificationController.php database/migrations/2026_08_02_210000_create_migration_qualification_tables.php database/seeders/ScreenshotGroup33DemoSeeder.php tests/Feature/Group33/MigrationQualificationTest.php
    ...
    & php -d memory_limit=1G vendor/bin/phpstan analyse app/Console/Commands/QualifyArchiveMigrationCommand.php app/Domain/CloudImport/Models/MigrationQualificationRun.php app/Domain/CloudImport/Services/ArchiveMigrationQualification.php app/Http/Controllers/Admin/MigrationQualificationController.php database/seeders/ScreenshotGroup33DemoSeeder.php --no-progress
    ...
    Write-Host 'OK  SG33 text contains no forbidden private or tooling references.' -ForegroundColor Green

    Write-Host "`n=== Repository evidence state ===" -ForegroundColor Cyan
    & git status --short

    Write-Host "`nScreenshot Group 33 validation commands passed." -ForegroundColor Green
    Write-Host 'Capture this PowerShell window as 06_v380_Validation.png.' -ForegroundColor Yellow
}
finally {
    Pop-Location
}
