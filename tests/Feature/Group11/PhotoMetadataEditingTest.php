...
function g11Owner(): User
{
    return User::factory()->create(['role' => 'owner', 'email_verified_at' => now()]);
}
function g11Item(User $owner, string $id = 'G11-001'): MediaItem
{
...
it('allows only the owner to open the edit form', function () {
    $owner = g11Owner();
    $item = g11Item($owner);
    $this->get(route('archive.photos.metadata.edit', $item))->assertRedirect('/login');
    $this->actingAs($owner)->get(route('archive.photos.metadata.edit', $item))->assertOk()->assertSee('Current metadata revision');
});
...
it('rejects stale and no-op submissions', function () {
    ...
    $this->actingAs($owner)->patch(route('archive.photos.metadata.update', $item), $payload)->assertSessionHasErrors('expected_metadata_revision');
    expect(PhotoMetadataRevision::count())->toBe(0);
});
