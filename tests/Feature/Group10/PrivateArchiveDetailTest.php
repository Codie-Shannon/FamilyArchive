...
it('shows safe read only detail through the web display derivative only', function () {
    ...
    $response->assertOk()
        ->assertSee('Safe archive title')
        ->assertSee('Approved description')
        ->assertSee('About this photo')
        ->assertSee('More date details')
        ->assertSee('Original safely preserved')
        ->assertSee('You are viewing an approved display copy. The original has not been changed.')
        ->assertSee('Technical details')
        ->assertSee('Source provenance')
        ->assertSee(route('archive.derivatives.preview', $web))
        ->assertDontSee($original->storage_path)
        ->assertDontSee($original->sha256)
...
it('fails safely for missing web display and rejects unapproved details', function () {
    ...
    $this->actingAs($owner)->get(route('archive.photos.show', $item))->assertOk()->assertSee('Web display unavailable')->assertSee('no generation side effect')->assertSee('The approved original has not been changed. A separate display copy is currently unavailable.');
