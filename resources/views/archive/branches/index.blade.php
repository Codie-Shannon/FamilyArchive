<x-layouts::app title="Reviewed Family Branches">
    <main class="mx-auto max-w-7xl space-y-6 p-6">
        <header class="flex flex-wrap items-end justify-between gap-4">
            ...
            <div class="flex gap-3"><a href="{{ route('archive.people.index') }}" class="rounded-lg border border-zinc-300 px-4 py-2 text-sm font-semibold dark:border-zinc-600 dark:text-white">People</a><a href="{{ route('archive.branches.create') }}" class="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-black">Add family branch</a></div>
        </header>
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @forelse($branches as $branch)
                <a href="{{ route('archive.branches.show', $branch) }}" class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
                    ...
                    <p class="mt-4 text-xs text-zinc-500">{{ $branch->people_count }} reviewed people · {{ $branch->provenance_links_count }} sources</p>
                </a>
            @empty
