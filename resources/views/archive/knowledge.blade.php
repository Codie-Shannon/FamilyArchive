    ...
    <main class="mx-auto max-w-7xl space-y-8 p-6">
        ...
        <section class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach($counts as $label => $count)
                <article class="rounded-xl border border-zinc-700 bg-zinc-900 p-5"><p class="text-sm capitalize text-zinc-400">{{ str($label)->replace('_', ' ') }}</p><p class="mt-2 text-3xl font-semibold text-white">{{ number_format($count) }}</p></article>
            @endforeach
        </section>
        <section class="rounded-xl border border-zinc-700 bg-zinc-900 p-6">
            ...
            <form class="mt-4 flex gap-3"><input name="q" value="{{ $query }}" maxlength="100" placeholder="Search people, events or safe locations" class="min-w-0 flex-1 rounded-lg bg-zinc-950 p-3 text-white"><button class="rounded-lg bg-emerald-500 px-5 font-semibold text-black">Search</button></form>
            <div class="mt-5 grid gap-3 md:grid-cols-2">
                @forelse($results as $result)
                    <article class="rounded-lg border border-zinc-700 p-4"><p class="text-xs uppercase tracking-wide text-emerald-300">{{ $result->entity_type }}</p><h3 class="mt-1 font-semibold text-white">{{ $result->label }}</h3><p class="font-mono text-xs text-zinc-500">{{ $result->stable_id }}</p></article>
                @empty
