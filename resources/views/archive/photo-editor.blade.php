...
<main class="mx-auto flex w-full max-w-[1600px] flex-1 flex-col gap-5 p-3 md:p-6" data-archive-photo-editor>
    ...
    <header class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        @if($current)
            <a href="{{ route('archive.photos.editor.split', [$current, 'return_to' => $returnTo, 'editor_return_to' => request()->getRequestUri()]) }}" data-editor-navigation class="rounded-xl border border-amber-600 px-4 py-3 font-semibold text-amber-100">Split photo</a>
        @endif
    </header>
...
(() => {
 ...
 const saveNow=()=>{if(!dirty)return Promise.resolve(true);if(saving)return saving;clearTimeout(timer);state.textContent='Saving draft…';const body=new FormData(form);body.set('_method','PUT');saving=(async()=>{try{const res=await fetch(form.action,{method:'POST',headers:{Accept:'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content},body});if(!res.ok){state.textContent=await errorMessage(res);return false}dirty=false;state.textContent='Draft autosaved.';if(publishCurrent)publishCurrent.disabled=false;if(publishAll)publishAll.disabled=false;if(!hasDraft){hasDraft=true;const count=Number(draftCountNode.textContent||0)+1;draftCountNode.textContent=String(count);draftLabel.textContent=count===1?'photo':'photos'}return true}catch{state.textContent='The server could not be reached. Your changes remain in this editor.';return false}finally{saving=null}})();return saving};
 const autosave=()=>{dirty=true;state.textContent='Draft has unsaved changes…';clearTimeout(timer);timer=setTimeout(saveNow,900)};
 ...
 document.querySelectorAll('[data-editor-navigation]').forEach(link=>link.addEventListener('click',async event=>{if(!dirty)return;event.preventDefault();if(await saveNow())location.assign(link.href)}));
 document.querySelectorAll('[data-publish-current-form],[data-publish-all-form]').forEach(publishForm=>publishForm.addEventListener('submit',async event=>{if(!dirty)return;event.preventDefault();if(await saveNow())publishForm.submit()}));
 controls.forEach(c=>c.addEventListener('input',()=>{render();autosave()}));document.querySelectorAll('[data-rotate]').forEach(b=>b.addEventListener('click',()=>{turn.value=Math.max(-2,Math.min(2,n('quarter_turn')+Number(b.dataset.rotate)));render();autosave()}));form.elements.from_source_scan?.addEventListener('change',()=>{dirty=true;form.submit()});document.querySelector('[data-save-draft]').addEventListener('click',()=>{dirty=true;saveNow()});document.querySelector('[data-reset-editor]').addEventListener('click',()=>{controls.forEach(c=>c.value=0);turn.value=0;render();autosave()});source.addEventListener('load',render);if(source.complete)render();window.addEventListener('beforeunload',event=>{if(dirty){event.preventDefault();event.returnValue=''}});
})();
</script>
@endif
</x-layouts::app>
