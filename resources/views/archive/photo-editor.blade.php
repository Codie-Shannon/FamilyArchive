...
<main class="mx-auto flex w-full max-w-[1600px] flex-1 flex-col gap-5 p-3 md:p-6" data-archive-photo-editor>
    ...
    @else
        <form id="archive-editor-form" method="POST" action="{{ route('archive.photos.editor.draft', $current) }}" class="grid min-h-0 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            @csrf @method('PUT')
            <input type="hidden" name="client_revision" value="{{ (int) ($draft?->client_revision ?? 0) + 1 }}">
            <div class="space-y-4">
    ...
    @if($batchEdit)
        ...
        <section data-photo-save-progress data-status-url="{{ route('archive.photos.editor.publish-all.status', $batchEdit) }}" class="rounded-2xl border border-sky-700 bg-sky-950/20 p-4" aria-live="polite">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div><strong data-save-progress-title class="text-white">{{ $batchEdit->isActive() ? 'Saving changed photos in the background' : ($batchEdit->failed_count > 0 ? 'Latest photo-save batch needs attention' : 'Latest photo-save batch complete') }}</strong><p data-save-progress-label class="mt-1 text-sm text-sky-100">{{ $batchProcessed }} of {{ $batchEdit->total_count }} processed@if($batchEdit->failed_count > 0); {{ $batchEdit->failed_count }} need retry@endif.@if($batchEdit->completed_at) Completed {{ $batchEdit->completed_at->diffForHumans() }}.@endif</p></div>
...
<script>
(() => {
 const panel=document.querySelector('[data-photo-save-progress]'),title=panel.querySelector('[data-save-progress-title]'),label=panel.querySelector('[data-save-progress-label]'),percent=panel.querySelector('[data-save-progress-percent]'),bar=panel.querySelector('[data-save-progress-bar]'),retry=panel.querySelector('[data-save-retry]'),refresh=panel.querySelector('[data-refresh-editor]');let polling;
 const render=data=>{percent.textContent=`${data.percent}%`;bar.style.width=`${data.percent}%`;title.textContent=data.active?'Saving changed photos in the background':data.failed?'Latest photo-save batch needs attention':'Latest photo-save batch complete';label.textContent=`${data.processed} of ${data.total} processed${data.failed?`; ${data.failed} need retry`:'.'}`;retry.hidden=!data.retryable;if(!data.active&&polling){clearInterval(polling);polling=null}};
...
(() => {
 ...
 const saveNow=(force=false)=>{if(!hasUnsaved()&&!force)return Promise.resolve(true);if(saving)return saving.then(ok=>ok&&hasUnsaved()?saveNow(force):ok);clearTimeout(timer);saving=(async()=>{try{let mustSend=force;while(hasUnsaved()||mustSend){mustSend=false;const snapshotRevision=changeRevision,expectedServerRevision=serverRevision+1,body=new FormData(form);body.set('_method','PUT');body.set('client_revision',String(expectedServerRevision));state.textContent='Saving draft…';const res=await fetch(form.action,{method:'POST',headers:{Accept:'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content},body});if(!res.ok){state.textContent=await errorMessage(res);return false}const acknowledgement=await res.json();if(Number(acknowledgement.client_revision)!==expectedServerRevision){state.textContent='The server did not acknowledge the latest editor revision. Reload before saving.';return false}serverRevision=expectedServerRevision;form.elements.client_revision.value=String(serverRevision+1);savedRevision=Math.max(savedRevision,snapshotRevision);if(!batchActive&&publishCurrent)publishCurrent.disabled=false;if(!batchActive&&publishAll)publishAll.disabled=false;if(!hasDraft){hasDraft=true;const count=Number(draftCountNode.textContent||0)+1;draftCountNode.textContent=String(count);draftLabel.textContent=count===1?'photo':'photos'}}state.textContent='Draft autosaved and acknowledged.';return true}catch{state.textContent='The server could not be reached. Your changes remain in this editor.';return false}finally{saving=null}})();return saving};
 ...
 document.querySelectorAll('[data-editor-navigation]').forEach(link=>link.addEventListener('click',async event=>{const preparesSplit=link.hasAttribute('data-prepare-split');if(!hasUnsaved()&&!hasDraft&&!preparesSplit)return;event.preventDefault();if((hasUnsaved()||hasDraft)&&!await saveNow(true))return;if(preparesSplit&&hasDraft&&!await publishDraft())return;location.assign(link.href)}));
 document.querySelectorAll('[data-publish-current-form],[data-publish-all-form]').forEach(publishForm=>publishForm.addEventListener('submit',async event=>{if(!hasDraft&&!hasUnsaved())return;event.preventDefault();if(await saveNow(true))publishForm.submit()}));
