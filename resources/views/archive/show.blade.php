...
<div class="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-6 p-4 md:p-8">
    <header><div class="flex flex-wrap items-center justify-between gap-3"><a href="{{ $returnTo }}" class="inline-flex items-center gap-2 text-sm font-medium text-emerald-300 hover:text-emerald-200">← Back to photos</a><div class="flex flex-wrap gap-3">@if(auth()->user()?->role === 'owner' || (int) $photo->createdBy === (int) auth()->id())<a href="{{ route('archive.photos.editor', ['single_photo' => $photo->mediaItemId, 'return_to' => request()->getRequestUri()]) }}" class="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-zinc-950">Edit photo</a>@endif @if(auth()->user()?->role === 'owner')<a href="{{ route('archive.photos.metadata.edit', $photo->mediaItemId) }}" class="px-2 py-2 text-sm font-medium text-emerald-300">Edit metadata</a><a href="{{ route('archive.photos.metadata.history', $photo->mediaItemId) }}" class="px-2 py-2 text-sm font-medium text-emerald-300">Revision history</a>@endif</div></div><p class="mt-5 text-xs font-semibold uppercase tracking-wider text-emerald-300">{{ $photo->archiveId }}</p><h1 class="mt-1 text-3xl font-semibold text-white">{{ $photo->title }}</h1></header>
