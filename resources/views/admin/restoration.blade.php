<x-layouts::app title="Restoration Automation">
    ...
    <main class="mx-auto max-w-7xl space-y-7 p-6">
        <header class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <p class="text-sm font-semibold text-emerald-300">{{ $focusedCandidateId ? 'Step 2 of 2 · Owner review' : 'Owner-controlled restoration' }}</p>
                <h1 class="mt-1 text-3xl font-semibold text-white">{{ $focusedCandidateId ? 'Original vs suggested edit' : 'Non-destructive restoration automation' }}</h1>
                <p class="mt-2 max-w-3xl text-zinc-400">
                    @if($focusedCandidateId)
                        Compare the preserved source with the generated edit. Approving changes the preferred viewing
                        version only; rejecting keeps the original as the viewing source.
                    @else
            ...
            <section class="space-y-4">
                <div class="flex flex-wrap items-end justify-between gap-3">
                    <div>
                        ...
                        <h2 class="mt-1 text-xl font-semibold text-white">{{ $focusedCandidateId ? 'Review generated edit' : 'Candidate queue' }}</h2>
                    </div>
                    @if($focusedCandidateId)
                        ...
                        @elseif($candidate)
                            <div class="mt-5 grid gap-4 md:grid-cols-2">
                                <figure class="overflow-hidden rounded-lg border border-zinc-700 bg-zinc-950">
                                    <img src="{{ route('admin.restoration.candidates.preview', [$candidate, 'source']) }}" alt="Immutable original preview" class="aspect-[4/3] w-full object-contain">
                                    <figcaption class="border-t border-zinc-700 px-3 py-2 text-xs text-zinc-300">Original — preserved unchanged</figcaption>
                                </figure>
                                <figure class="overflow-hidden rounded-lg border border-emerald-800 bg-zinc-950">
                                    <img src="{{ route('admin.restoration.candidates.preview', [$candidate, 'candidate']) }}" alt="Suggested restoration preview" class="aspect-[4/3] w-full object-contain">
                                    <figcaption class="border-t border-emerald-800 px-3 py-2 text-xs text-emerald-200">Suggested edit — approve or reject</figcaption>
                                </figure>
                            </div>
                            ...
                            @if($candidate->review_state === 'pending')
                                <form method="POST" action="{{ route('admin.restoration.candidates.review', $candidate) }}" class="mt-4 grid gap-3 sm:grid-cols-[1fr_auto_auto]">
                                    ...
                                    <button name="decision" value="approved" class="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-zinc-950">Approve suggested edit</button>
                                    <button name="decision" value="rejected" class="rounded-lg border border-red-800 px-4 py-2 text-sm font-semibold text-red-200">Reject suggested edit</button>
                                </form>
                            @else
