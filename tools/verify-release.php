...
$pass($version === '4.2.0', 'Canonical version must be 4.2.0.');
$pass($releaseName === 'Batch Content Safeguards', 'Canonical release name must be Batch Content Safeguards.');
$pass(($state['status'] ?? null) === 'approved', 'Canonical release status must be approved.');
$pass($groupCount === 37 && $latestGroup === 37, 'Canonical screenshot-group state must close at SG37.');
...
$pass(str_contains((string) config('release.status'), '37'), 'config/release.php does not identify SG37 as closed.');
...
$releaseNotes = $read('docs/release-notes/v4.2.0.md');
...
foreach ([$readme, $roadmap, $overview, $releaseNotes] as $document) {
    $pass(str_contains($document, '4.2.0'), 'A primary release document does not identify v4.2.0.');
    $pass(str_contains($document, '37'), 'A primary release document does not identify SG37.');
}
